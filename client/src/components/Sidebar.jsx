...
const Sidebar = () => {
  ...
  return (
    <div className={`bg-[#11101D] max-w-[340px] w-full h-full p-5 rounded-r-xl overflow-y-scroll
        ...
        <div className='flex flex-col gap-2'>
          {filteredUsers.map((user, index) => (
            <div
              ...
              cursor-pointer max-sm:text-sm transition-all duration-300 ${selectedUser?._id === user._id ? 'bg-[#282142]/50' : 'hover:bg-[#282142]/20'}`}
            >
              ...
              {unseenMessages[user._id] && (
                <p className='absolute top-3 right-4 text-xs h-5 w-5
                flex justify-center items-center rounded-full bg-violet-500/70 text-white'>
                  {unseenMessages[user._id]}
                </p>
              )}
            </div>
